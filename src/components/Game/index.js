...
export default class Game extends Component {
  ...
  matchCards(card) {
    if (
      card.props.isMatched ||
      card.props.isFlipped ||
      this.state.isboardLocked
    )
      return
  ...
  prepareCards(cardButtons, count) {
    ...
    let cards = [...this.getRandomIndices(iconsetLength - 1, 0, halfCount)].map(
      cardIndex => cardButtons[cardIndex],
    )
    return [...this.getRandomIndices(count - 1, 0, count)].map(
      (index, seqIdx) => {
